...
const categories = [
  { id: "pizza", name: "Pizzas", items: [
    ["Margherita Pizza", 7.0, "With pizza sauce & mozzarella cheese."],
  ...
  ] },
 {
  ...
  { id: "snacks", name: "Quick Snacks", items: [
    ...
    ["Caspian Chips", 4.5, "Pakora battered chips with spice."],
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white selection:bg-orange-500/40">
      ...
      {page === "home" && <HomePage go={go} settings={settings} shopStatus={shopStatus} />}
        ...
        {page === "privacy" && <PrivacyPolicyPage settings={settings} />}
        {page === "terms" && <TermsPage settings={settings} />}
        {page === "refunds" && <RefundPolicyPage settings={settings} />}
        {page === "delivery" && <DeliveryPolicyPage settings={settings} />}
...
function ContactPage({ contactSent, setContactSent }) { return <section className="mx-auto max-w-7xl px-5 pb-24 pt-40 lg:px-8"><div className="mx-auto max-w-3xl text-center"><p className="mb-5 text-sm font-black uppercase tracking-[0.42em] text-[#ff5b00]">Get in touch</p><h1 className="font-serif text-6xl font-black">Contact Us</h1><p className="mt-6 text-xl leading-8 text-white/70">Have a question or feedback? We'd love to hear from you. Reach out and we'll get back to you as soon as possible.</p></div><div className="mt-24 grid gap-12 lg:grid-cols-[1fr_1fr]"><div><div className="grid gap-5 sm:grid-cols-2"><ContactCard icon="pin" title="Address" text={<>26 Main Street<br />Kelty, KY4 0AA</>} /><ContactCard icon="phone" title="Phone" text="01383 830 166" /><ContactCard icon="mail" title="Email" text="hello@caspiantandoori.com" /><ContactCard icon="clock" title="Opening Hours" text={<>Mon-Thu: 4PM - 12AM<br />Fri-Sat: 4PM - 1AM</>} /></div><div className="mt-8 overflow-hidden rounded-2xl border border-white/10"><iframe title="Caspian map" src="https://maps.google.com/maps?q=Caspian Tandoori Kelty&output=embed" className="h-80 w-full border-0" loading="lazy" /></div></div><div className="rounded-[1.75rem] border border-white/10 bg-[#101010] p-8 lg:p-10"><h2 className="font-serif text-3xl font-black">Send us a Message</h2>{contactSent ? <div className="mt-8 rounded-2xl border border-green-500/20 bg-green-500/10 p-5 text-green-200">Thanks. Your message has been received and our team will get back to you shortly.</div> : <form onSubmit={(e) => { e.preventDefault(); setContactSent(true); }} className="mt-7 grid gap-6"><Field label="Your Name *" placeholder="John Doe" required /><Field label="Email Address *" placeholder="john@example.com" required type="email" /><Field label="Phone Number (Optional)" placeholder="07123 456789" /><label className="grid gap-2 text-sm font-bold">Message *<textarea required placeholder="How can we help you?" className="min-h-36 rounded-lg border border-white/10 bg-white/7 p-4 outline-none focus:ring-2 focus:ring-[#ff5b00]" /></label><Button type="submit" className="rounded-full bg-[#ff5b00] py-4 font-black text-white hover:bg-orange-600"><Icon name="send" className="mr-3" /> Send Message</Button></form>}</div></div></section>; }
function LegalPage({ title, intro, sections }) { return <section className="mx-auto max-w-4xl px-5 pb-24 pt-40 lg:px-8"><p className="mb-4 text-sm font-black uppercase tracking-[0.35em] text-[#ff5b00]">Customer information</p><h1 className="font-serif text-5xl font-black md:text-6xl">{title}</h1><p className="mt-6 text-lg leading-8 text-white/70">{intro}</p><div className="mt-10 grid gap-6">{sections.map((section) => <div key={section.title} className="rounded-2xl border border-white/10 bg-white/[0.04] p-7"><h2 className="font-serif text-2xl font-black">{section.title}</h2><p className="mt-4 leading-8 text-white/68">{section.text}</p></div>)}</div><p className="mt-10 text-sm text-white/45">Last updated: 14 June 2026.</p></section>; }
function PrivacyPolicyPage({ settings }) { const name = settings?.restaurantName || "Caspian Tandoori"; return <LegalPage title="Privacy Policy" intro={`${name} collects only the customer information needed to process takeaway orders, contact customers about orders, and manage customer accounts.`} sections={[{ title: "Information we collect", text: "We may collect your name, email address, phone number, delivery address, order notes, saved addresses, and order history when you place an order or create an account." }, { title: "Payments", text: "Online card payments are handled securely by Stripe. We do not store full card numbers or card security codes on this website." }, { title: "How we use information", text: "We use customer information to prepare orders, arrange collection or delivery, respond to enquiries, provide receipts, manage offers, and maintain account security." }, { title: "Contact", text: `For privacy questions, contact ${name} at ${settings?.email || "hello@caspiantandoori.com"} or ${settings?.phone || "01383 830 166"}.` }]} />; }
function TermsPage({ settings }) { const name = settings?.restaurantName || "Caspian Tandoori"; return <LegalPage title="Terms of Service" intro={`These terms apply when you use the ${name} website to browse the menu, create an account, or place a collection or delivery order.`} sections={[{ title: "Orders", text: "Please check your basket, order type, delivery address, allergies, and contact details before paying. We may contact you if an item is unavailable or order details need clarification." }, { title: "Prices and availability", text: "Prices are shown in GBP and may change from time to time. Menu items, offers, opening hours, delivery areas, and preparation times are subject to availability." }, { title: "Allergies", text: "Please add allergy information in the order notes and contact the restaurant before ordering if you have a severe allergy. Food is prepared in a kitchen that handles multiple allergens." }, { title: "Account security", text: "Customers are responsible for keeping their login details secure and for notifying us if they believe their account has been used without permission." }]} />; }
function RefundPolicyPage({ settings }) { const name = settings?.restaurantName || "Caspian Tandoori"; return <LegalPage title="Refunds & Cancellations" intro={`${name} wants every order to be correct. Please contact the restaurant as soon as possible if there is a problem with your order.`} sections={[{ title: "Cancelling an order", text: "Orders can usually be cancelled before the kitchen starts preparing them. Once preparation has started, cancellation may not be possible because food is made to order." }, { title: "Refunds", text: "Refunds may be offered for duplicate payments, cancelled orders accepted by the restaurant, unavailable items, or confirmed order problems. Approved card refunds are returned to the original payment method." }, { title: "Order problems", text: "If an item is missing, incorrect, or not suitable, contact us promptly with your order details so the team can review it and arrange a suitable resolution." }, { title: "Contact", text: `Call ${settings?.phone || "01383 830 166"} for urgent order issues.` }]} />; }
function DeliveryPolicyPage({ settings }) { const zones = settings?.deliveryZonesText || "Kelty, Cowdenbeath, Kinross, Lochgelly, Ballingry, Cardenden"; return <LegalPage title="Delivery Information" intro="Delivery is available to selected local areas. The basket checks your address before checkout and shows the delivery fee where delivery is available." sections={[{ title: "Delivery areas", text: `Current delivery areas include: ${zones}. Please include your full address and postcode so we can confirm availability.` }, { title: "Fees and minimum order", text: `Delivery fees are shown before checkout. Minimum delivery order: £${Number(settings?.minimumOrder || 0).toFixed(2)}.` }, { title: "Times", text: "Delivery times are estimates and may vary during busy periods, bad weather, or traffic delays. Scheduled orders are prepared as close as possible to the selected time." }, { title: "Collection", text: "Collection orders can be picked up from 26 Main Street, Kelty, KY4 0AA unless the restaurant confirms otherwise." }]} />; }
